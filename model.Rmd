...
pitches %<>% 
  ...
  mutate(score_diff = fld_score - bat_score,
         ...
         on_3b = as.integer(!is.na(on_3b)),
         on_2b = as.integer(!is.na(on_2b)),
         on_1b = as.integer(!is.na(on_1b)),
...
First, we'll create a prelimary linear model to predict wOBA solely from pitch sequence, focusing on those sequences that have appeared at least 2\% of the time.
...
# we also turn the pitch sequences into factors here

pitches_mod <- pitches %>%
  filter(pitch_seq %in% sequences, woba_denom == 1) %>% 
  mutate(pitch_seq = as_factor(pitch_seq))
